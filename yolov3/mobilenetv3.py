...
if __name__ == '__main__':
    ...
    net.features[3].register_forward_hook(get_features)
    net.features[8].register_forward_hook(get_features)
    net.features[13].register_forward_hook(get_features)

    #print('mobilenetv3:\n', net)
    print('Total params: %.2fM' % (sum(p.numel() for p in net.parameters())/1000000.0))

    input_size=(2, 3, 416, 416)
    x = torch.randn(input_size)
    y = net(x)

    for i in range(len(features)):
        print(i, features[i].size())
