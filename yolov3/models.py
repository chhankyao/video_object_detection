...
class DarknetConv2D(nn.Module):
    def __init__(self, inp, oup, filter_size):
        super(DarknetConv2D, self).__init__()
        pad = (filter_size - 1) // 2
        '''self.layers = nn.Sequential(
            nn.Conv2d(inp, oup, filter_size, padding=pad, bias=False),
            nn.BatchNorm2d(oup),
            nn.LeakyReLU(0.1),
        )
        '''
        self.layers = nn.Sequential(
            nn.Conv2d(inp, inp, filter_size, padding=pad, bias=False, groups=inp),
            nn.BatchNorm2d(inp),
            nn.LeakyReLU(0.1),
...
class LastLayers(nn.Module):
    def __init__(self, inp, channels, oup):
        super(LastLayers, self).__init__()
        self.layers_x = nn.Sequential(
            DarknetConv2D(inp, channels, 1),
            DarknetConv2D(channels, channels * 2, 3),
            DarknetConv2D(channels * 2, channels, 1),
            DarknetConv2D(channels, channels * 2, 3),
            DarknetConv2D(channels * 2, channels, 1),
        )
        self.layers_y = nn.Sequential(
            DarknetConv2D(inp, channels * 2, 3),
            nn.Conv2d(channels * 2, oup, 1)
        )

    def forward(self, x):
        return self.layers_x(x), self.layers_y(x)


class YOLOHead(nn.Module):
    def __init__(self, oup):
        super(YOLOHead, self).__init__()
        self.last_layers1 = LastLayers(576, 288, oup)
        self.conv1 = DarknetConv2D(288, 144, 1)
        self.last_layers2 = LastLayers(192, 96, oup)
        self.conv2 = DarknetConv2D(96, 32, 1)
        self.last_layers3 = LastLayers(56, 28, oup)

    def forward(self, f1, f2, f3):
        x, y1 = self.last_layers1(f1)
        x = self.conv1(x)
        x = F.interpolate(x, scale_factor=2, mode='bilinear')

        x = torch.cat([x, f2], 1)
        x, y2 = self.last_layers2(x)
        x = self.conv2(x)
        x = F.interpolate(x, scale_factor=2, mode='bilinear')

        x = torch.cat([x, f3], 1)
        x, y3 = self.last_layers3(x)
        return y1, y2, y3
...
class Darknet(nn.Module):
    ...
    def get_features(self, x, input, output):
        global features
    ...
    def __init__(self, cfg, img_size=(416, 416)):
        ...
        '''self.mobilenet = mobilenetv3(n_class=80, input_size=416, mode='small')
        state_dict = torch.load('weights/mobilenetv3_small_67.218.pth.tar')
        state = self.mobilenet.state_dict()
        for name, param in state_dict.items():
            if name in state:
                state[name].copy_(param)
        '''
        '''self.mobilenet.features[3].register_forward_hook(self.get_features)
        self.mobilenet.features[8].register_forward_hook(self.get_features)
        self.mobilenet.features[13].register_forward_hook(self.get_features)
        '''
        # self.yolo_head = YOLOHead(3 * (80 + 5))
    ...
    def forward(self, x, var=None):
        ...
        f3, f2, f1 = self.mobilenet(x)
        #_ = self.mobilenet(x)
        #f3, f2, f1 = features

        model_outputs = self.yolo_head(f1, f2, f3)
        '''
        for i, (module_def, module) in enumerate(zip(self.module_defs, self.module_list)):
            ...
            if mtype in ['convolutional', 'upsample', 'maxpool']:
                ...
                # x = module[0](model_outputs[i], img_size)
                output.append(x)
