...
class VOD(nn.Module):
    ...
    def get_reward(self):
        ...
        #self.rewards.append(r - r2)
        #if len(self.rewards) > self.scheduler.base_interval:
        #    del self.rewards[0]
        #reward = np.mean(self.rewards)
        cost = np.sum(self.scheduler.detect_frames)
        if cost > (self.scheduler.base_interval/5):
            return -100
    ...
    def step(self, a):
        ...
        if detect:
            ...
            for i in range(len(self.boxes)):
                self.trackers.append(kcftracker.KCFTracker(False, True, True))
                self.trackers[-1].init(to_tracking_box(self.boxes[i][:4]), self.frame)

        r = self.get_reward()
        done = (self.frame_i == len(self.img_list)-2) or (len(self.boxes) == 0) or (r == -100)
